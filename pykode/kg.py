...
def statistikk():
    # Henter  tilgjengelige kommuner fra kgstatistikk.py
    kommuneliste = hent_kommuneliste()

    if request.method == 'POST':
        # Brukeren sender inn et valg
        valgt_kommune = request.form.get('kommune', '')

        if not valgt_kommune:
            feil = "Vennligst velg en kommune."
            return render_template('statistikk_resultat.html', kommune="", feil=feil)

        # Generer statistikk for den valgte kommunen
        img = generer_statistikk(valgt_kommune)
        if img:
            return render_template('statistikk_resultat.html', kommune=valgt_kommune, plot_url=img)
        else:
            feil = f"Ingen data tilgjengelig for {valgt_kommune}."
            return render_template('statistikk_resultat.html', kommune=valgt_kommune, feil=feil)

    # GET: Vis siden for valg av kommune
    return render_template('statistikk.html', kommuneliste=kommuneliste)
